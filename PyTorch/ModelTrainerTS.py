...
IMG_DIR = "/scratch/datasets/QuantLab/ImageNet/data/train/"

class AugLoader:
    def __init__(self, h, w, bs=64, img_dir=IMG_DIR):
        self.train_dataset = \
            datasets.ImageFolder(img_dir,
        ...
        self.train_loader = torch.utils.data.DataLoader(
            self.train_dataset, batch_size=bs, sampler=None, shuffle=True
        )

class ModelTrainer:
    def __init__(self, model, args, regime, teacher=False):
        ...
        # init prefc
        # self.model.prefc.weight.data[:] = 1./128
    ...
    def Deploy(self, validation_loader, h, w, prec_dict=None):
        ...
        bin_qd, bout_qd, (valid_loss_x, valid_loss_y, valid_loss_z, valid_loss_phi, y_pred, gt_labels) = nemo.utils.get_intermediate_activations(self.model, self.ValidateSingleEpoch, validation_loader)
        acc = float(1) / (valid_loss_x + valid_loss_y + valid_loss_z + valid_loss_phi)
        logging.info("[ModelTrainer]: QuantizedDeployable network: %f" % acc)

        # id_stage requires NEMO>=0.0.3
        self.model.id_stage()
        bin_id, bout_id, (valid_loss_x, valid_loss_y, valid_loss_z, valid_loss_phi, y_pred, gt_labels) = nemo.utils.get_intermediate_activations(self.model, self.ValidateSingleEpoch, validation_loader, integer=True)
        acc = float(1) / (valid_loss_x + valid_loss_y + valid_loss_z + valid_loss_phi)
    ...
    def TrainQuantized(self, train_loader, validation_loader, h, w, epochs=100, relaxation=False):
        ...
        for epoch in range(1, epochs):
            ...
            # self.optimizer.param_groups[0]['lr'] = float(self.regime['lr']) * lr_factor
            # self.optimizer.param_groups[1]['lr'] = float(self.regime['lr']) * lr_factor
            # UNCOMMENT FOR ALTERNATE TRAININ
            # if train_qnt:
            #     self.optimizer.param_groups[0]['lr'] = float(self.regime['lr']) * lr_factor
            ...
            train_loss_x, train_loss_y, train_loss_z, train_loss_phi = self.TrainSingleEpoch(train_loader, aug_loader)
            ...
            if acc > best:
                nemo.utils.save_checkpoint(self.model, self.optimizer, epoch, acc, checkpoint_name='HannaNet_', checkpoint_suffix='best')
                best = acc

            if self.ipython:
                import IPython; IPython.embed()
        nemo.utils.save_checkpoint(self.model, self.optimizer, epoch, acc, checkpoint_name='HannaNet_', checkpoint_suffix='final')

    def TrainSingleEpoch(self, training_generator, aug_loader=None, nb_aug=100, augmented=False, aug_lr_factor=0.1):
        ...
        i = 0
        def single_iter(batch_samples, batch_targets):
        ...
        if augmented:
            for pg in self.optimizer.param_groups:
                pg['lr'] *= aug_lr_factor
            with tqdm(total=nb_aug,
                desc='Augm ') as t:
                for i, (batch_samples, batch_targets) in enumerate(aug_loader.train_loader):
                    if i >= nb_aug:
                        break
                    batch_samples = batch_samples.to(self.device)
                    batch_targets = self.teacher(batch_samples)
                    batch_targets = torch.cat(batch_targets, 1)
                    loss = single_iter(batch_samples, batch_targets)
                    t.set_postfix({'loss': loss})
                    t.update(1)
            for i,pg in enumerate(self.optimizer.param_groups):
                pg['lr'] = lr[i]
 
        with tqdm(total=len(training_generator),
            desc='Train') as t:
        ...
        with torch.no_grad():
            for batch_samples, batch_targets in validation_generator:
                ...
                if integer:
                    ...
                    outputs = [ (o-self.model.fc.bias[i]) * eps_fcout + self.model.fc.bias[i] for i,o in enumerate(outputs) ]
            ...
            logging.info("[ModelTrainer] Mean validation loss {:2f}: x={:2f}, y={:2f}, z={:2f}, phi={:2f}".format(valid_loss['tot'].value, valid_loss['x'].value, valid_loss['y'].value,
                                                                                                   valid_loss['z'].value,
                                                                                                   valid_loss['phi'].value))
            logging.info("[ModelTrainer] MSE {:2f}: x={:2f}, y={:2f}, z={:2f}, phi={:2f}".format(mse_loss['tot'].value, mse_loss['x'].value, mse_loss['y'].value,
                                                                                  mse_loss['z'].value,
                                                                                  mse_loss['phi'].value))
            logging.info("[ModelTrainer] MAE {:2f}: x={:2f}, y={:2f}, z={:2f}, phi={:2f}".format(mae_loss['tot'].value, mae_loss['x'].value, mae_loss['y'].value,
                                                                                  mae_loss['z'].value,
                                                                                  mae_loss['phi'].value))


        return valid_loss['x'].value, valid_loss['y'].value, valid_loss['z'].value, valid_loss['phi'].value, y_pred, gt_labels
    ...
    def Train(self, training_generator, validation_generator):
        ...
        for epoch in range(1, self.args.epochs + 1):
            ...
            logging.info('[ModelTrainer] Validation MSE: {:2f}'.format(MSE))
            logging.info('[ModelTrainer] Validation MAE: {:2f}'.format(MAE))
            logging.info('[ModelTrainer] Validation r2_score: {:2f}'.format(r2_score))
